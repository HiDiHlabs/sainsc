...
use ndarray::Array2;
use num::{one, zero, Zero};
use numpy::{IntoPyArray, PyArray2, PyReadonlyArray2};
use polars::{
    datatypes::{
        CategoricalOrdering::Physical,
        DataType::{Categorical, Int32},
    },
    prelude::*,
};
use pyo3::{
...
pub(crate) type CsxIndex = i32;

/// Class implementation

macro_rules! build_GridClass {
        ...
        impl $name {
            ...
            fn from_dataframe(
                ...
                fn _from_dataframe(
                    ...
                    (
                        HashMap<String, CsMatI<$t, CsxIndex, CsxIndex>>,
                        (usize, usize),
                    ),
                    PolarsError,
                > {
                    ...
                    match df.column("count") {
                        ...
                        Err(_) => {
                            df.with_column(Series::new("count", vec![one::<$t>(); df.height()]))?
                        }
                        Ok(s) => df.with_column(
                            s.strict_cast(&(<$t_pl as PolarsDataType>::get_dtype()))?,
                        )?,
                    };
                    ...
                    let counts_dict = df
                        ...
                        .map(|df| {
                            ...
                            let counts = col_as_nonull_vec(&df, "count", |s| s.unpack::<$t_pl>())?;
            ...
            fn __getitem__(&self, key: String) -> PyResult<WrappedCsx<$t, CsxIndex, CsxIndex>> {
            ...
            fn __setitem__(&mut self, key: String, value: WrappedCsx<$t, CsxIndex, CsxIndex>) {
            ...
            fn __ne__(&self, other: &$name) -> bool {
                !self.__eq__(other)
            }
            ...
            fn __getnewargs_ex__(
                ...
                (HashMap<String, WrappedCsx<$t, CsxIndex, CsxIndex>>,),
            ...
            fn __str__(&self) -> String {
                let repr = vec![
                    format!("{} ({} threads)", stringify!($name), self.n_threads),
            ...
            fn get(
                &self,
                key: String,
                default: Option<WrappedCsx<$t, CsxIndex, CsxIndex>>,
            ) -> Option<WrappedCsx<$t, CsxIndex, CsxIndex>> {
            ...
            fn gene_counts(&self) -> HashMap<String, $t> {
                self.threadpool.install(|| {
                    self.counts
                        .par_iter()
            ...
            fn grid_counts(&self) -> Py<PyArray2<$t>> {
            ...
            #[pyo3(signature = (min=zero(), max=$t::MAX))]
            fn filter_genes_by_count(&mut self, min: $t, max: $t) {
...
build_GridClass!(GridCounts, u32, UInt32Type);
build_GridClass!(GridFloats, f32, Float32Type);
